import { worlds, materials, events, filters } from './data/worlds.js';
import './style.css';

const app = document.querySelector('#app');
const state = { selected: worlds[0], filter: 'All worlds', query: '', material: null, event: null, environment: worlds[0].environment, intensity: 58, scale: 0, time: '1×', running: false, active: 'World', activeEvent: false, sound: false };
state.material = materials[0].name;
    ...
    <main class="layout">
      <section class="experience">
        <div class="scene-wrap" style="--tone:${w.palette[0]}"><canvas id="scene" style="transform:scale(${[1,1.35,1.7,2][state.scale]})"></canvas><div class="scene-grain"></div><div class="scene-vignette"></div>
          ...
          <div class="scene-controls"><button class="scene-ctrl" id="view-scale"><span>⊙</span><span>${['WORLD','BUILDING','MATERIAL','FIBER'][state.scale]}</span></button><span class="scene-ctrl-sep"></span><button class="scene-ctrl" id="random-world"><span>⤨</span><span>Surprise me</span></button><span class="scene-ctrl-sep"></span><span class="scene-hint">DRAG TO LOOK AROUND <span>↔</span></span></div>
        </div>
        <div class="explore-strip"><div class="strip-intro"><span class="eyebrow">A UNIVERSE OF POSSIBILITIES</span><span class="strip-count">${String(worlds.length).padStart(2,'0')} WORLDS</span></div><div class="category-row">${filters.map(f=>`<button class="filter-chip ${state.filter===f?'active':''}" data-filter="${f}">${f}</button>`).join('')}</div></div>
      ...
      <aside class="control-rail"><div class="rail-header"><div><span class="eyebrow">WORLD BUILDER <span class="tiny-live"></span></span><h2>Change the <em>rules.</em></h2></div><button class="reset-button" id="reset">↺</button></div>
        ...
        <div class="rule-stack">
          ...
          <div class="event-chain"><span class="chain-node active"><i>01</i>${esc(state.event)}</span><span class="chain-arrow">↓</span><button class="chain-add" id="chain-add">+ Add an event</button></div>
        ...
        <div class="rail-bottom"><div class="presence-indicator"><span></span> PREVIEW READY <small>WEB</small></div><button class="build-button" id="build-world"><span>${state.activeEvent?'Rebuild this world':'Build this world'}</span><b>↗</b></button><div class="rail-footnote">${worlds.length} REALITIES <span>·</span> ∞ POSSIBLE COMBINATIONS</div></div>
      </aside>
    </main>
...
function chooseWorld(id){state.selected=worlds.find(x=>x.id===id)||worlds[0];state.event=state.selected.event;state.environment=state.selected.environment;state.activeEvent=false;render();}
function bind(){
  ...
  document.querySelector('#material-select').onchange=e=>{state.material=e.target.value;let m=materials.find(x=>x.name===state.material);document.querySelector('.material-meter span:first-child').textContent=m.family;document.querySelector('.material-meter i').style.width=m.stiffness+'%';toast(`${m.name} is now shaping this world`);};
  document.querySelector('#intensity').oninput=e=>{state.intensity=+e.target.value;document.querySelector('#intensity-label').innerHTML=`${state.intensity}<small>%</small>`;e.target.style.setProperty('--range',state.intensity+'%');document.querySelector('.scene-wrap').style.setProperty('--event-intensity',state.intensity/100);};
  ...
  document.querySelector('#slower').onclick=()=>{let ix=['⏸','0.25×','1×','10×','100×'].indexOf(state.time);state.time=['⏸','0.25×','1×','10×','100×'][Math.max(0,ix-1)];render();};
  document.querySelector('#faster').onclick=()=>{let ix=['⏸','0.25×','1×','10×','100×'].indexOf(state.time);state.time=['⏸','0.25×','1×','10×','100×'][Math.min(4,ix+1)];render();};
  document.querySelector('#random-world').onclick=()=>{let pool=worlds.filter(x=>x.id!==state.selected.id);chooseWorld(pool[Math.floor(Math.random()*pool.length)].id);toast('A new reality is ready to explore');};
  ...
  document.querySelector('#build-world').onclick=()=>{state.activeEvent=true;state.running=true;document.querySelector('#time-toggle').classList.add('on');document.querySelector('#time-toggle').innerHTML='<span></span>RUNNING';document.querySelector('#build-world span').textContent='Rebuild this world';drawMain();toast(`${state.event} is unfolding · inspect the changing world`);};
  document.querySelector('#reset').onclick=()=>{state.material=materials[0].name;state.event=state.selected.event;state.environment=state.selected.environment;state.intensity=58;state.scale=0;state.time='1×';state.running=false;state.activeEvent=false;render();toast('Rules restored to this world’s starting conditions');};
  ...
  document.querySelectorAll('[data-add-event]').forEach(b=>b.onclick=()=>{state.event=`${state.event} → ${b.dataset.addEvent}`;const chain=document.querySelector('.event-chain');const eventsInChain=state.event.split(' → ');chain.innerHTML=eventsInChain.map((ev,i)=>`${i?'<span class="chain-arrow">↓</span>':''}<span class="chain-node ${i===eventsInChain.length-1?'active':''}"><i>${String(i+1).padStart(2,'0')}</i>${esc(ev)}</span>`).join('')+`<button class="chain-add" id="chain-add">+ Add an event</button>`;document.querySelector('#chain-add').onclick=()=>document.querySelector('#event-modal').classList.add('open');document.querySelector('#event-modal').classList.remove('open');toast(`${b.dataset.addEvent} added to the event sequence`);});
  ...
  document.querySelector('#environment').onclick=()=>{const climates=['Dry','Humid','Saturated','Frozen','Salt air','Windy','Arid','Controlled'];const current=state.environment.split('·')[0].trim();const next=climates[(climates.indexOf(current)+1)%climates.length];const temp=state.environment.split('·')[1]?.trim()||'22°C';state.environment=`${next} · ${temp}`;render();toast(`Atmosphere changed to ${next.toLowerCase()}`);};
  document.querySelector('#temperature').onclick=()=>{const temps=['12°C','20°C','29°C','36°C','43°C','−18°C'];const current=state.environment.split('·')[1]?.trim();const next=temps[(temps.indexOf(current)+1)%temps.length];state.environment=`${state.environment.split('·')[0].trim()} · ${next}`;render();toast(`Temperature set to ${next}`);};
  ...
  document.querySelector('#exit-immersive').onclick=()=>{if(document.fullscreenElement)document.exitFullscreen();document.querySelector('#immersive-hud').classList.remove('visible');};
...
async function enterPreview(){const hud=document.querySelector('#immersive-hud');document.querySelector('#hud-world').textContent=state.selected.name;hud.classList.add('visible');try{await document.documentElement.requestFullscreen?.();}catch{}toast('World preview opened · immersive WebXR rendering is the next layer');}
